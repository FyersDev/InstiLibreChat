...
const initializeAgent = async ({
  ...
  // Load Default Financial Analyst System Prompt from file
  // You can customize the prompt by editing: api/server/services/Endpoints/agents/default-financial-prompt.txt
  // Or set a custom path via environment variable: DEFAULT_FINANCIAL_PROMPT_PATH
  let defaultFinancialPrompt = '';
  try {
    const promptFilePath = process.env.DEFAULT_FINANCIAL_PROMPT_PATH || 
      path.join(__dirname, 'default-financial-prompt.txt');
    defaultFinancialPrompt = fs.readFileSync(promptFilePath, 'utf8').trim();
  } catch (error) {
    logger.warn('Failed to load default financial prompt from file, using fallback:', error.message);
    // Fallback prompt if file cannot be read
    defaultFinancialPrompt = `You are a senior Financial Analyst and FinTech domain expert. 
